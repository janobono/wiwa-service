...
public enum MetaColumnWiwaBoardProductView {
    ...
    MetaColumnWiwaBoardProductView(final String columnName, final DataType dataType) {
        this.columnName = columnName;
        this.dataType = dataType;
    }

    public Column column() {
        return Column.column(columnName, dataType, MetaTable.WIWA_BOARD_PRODUCT_VIEW.table());
    }

    public Column column(final String tableAlias) {
        return Column.column(columnName, dataType, MetaTable.WIWA_BOARD_PRODUCT_VIEW.table(tableAlias));
    }

    public static Column[] columns() {
        final List<Column> columnList = new ArrayList<>();
        for (final MetaColumnWiwaBoardProductView metaColumnWiwaBoardProductView : values()) {
            columnList.add(metaColumnWiwaBoardProductView.column());
        }
    ...
    public static Column[] columns(final String tableAlias) {
        final List<Column> columnList = new ArrayList<>();
        for (final MetaColumnWiwaBoardProductView metaColumnWiwaBoardProductView : values()) {
            columnList.add(metaColumnWiwaBoardProductView.column(tableAlias));
        }
        return columnList.toArray(new Column[0]);
    }
}

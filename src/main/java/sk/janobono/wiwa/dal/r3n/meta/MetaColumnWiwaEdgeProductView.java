...
public enum MetaColumnWiwaEdgeProductView {
    ...
    MetaColumnWiwaEdgeProductView(final String columnName, final DataType dataType) {
        this.columnName = columnName;
        this.dataType = dataType;
    }

    public Column column() {
        return Column.column(columnName, dataType, MetaTable.WIWA_EDGE_PRODUCT_VIEW.table());
    }

    public Column column(final String tableAlias) {
        return Column.column(columnName, dataType, MetaTable.WIWA_EDGE_PRODUCT_VIEW.table(tableAlias));
    }

    public static Column[] columns() {
        final List<Column> columnList = new ArrayList<>();
        for (final MetaColumnWiwaEdgeProductView metaColumnWiwaEdgeProductView : values()) {
            columnList.add(metaColumnWiwaEdgeProductView.column());
        }
    ...
    public static Column[] columns(final String tableAlias) {
        final List<Column> columnList = new ArrayList<>();
        for (final MetaColumnWiwaEdgeProductView metaColumnWiwaEdgeProductView : values()) {
            columnList.add(metaColumnWiwaEdgeProductView.column(tableAlias));
        }
        return columnList.toArray(new Column[0]);
    }
}
